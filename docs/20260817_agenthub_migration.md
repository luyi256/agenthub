...
- 顶部新增历史搜索入口，可搜索当前 VS Code 项目下所有用户消息和 Agent 回复，包含已关闭会话；支持按消息角色过滤、关键词高亮、全文展开，并可跳回仍可用的所属会话。
- 输入框识别 IME composition 生命周期；中文输入法中用于上屏或直接输入英文的 Enter 不会再误触发送，普通 Enter 仍发送，Shift+Enter 仍换行。
